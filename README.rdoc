...
While elastic search automatically maps fields, you may wish to override the defaults:
  ...
  class Product < YourFavoriteOrm::Base
    elastic_index.configure do
      property :status, type: "date", index: "not_analyzed"
    end
  end

You can also directly access Product.elastic_index.mapping and Product.elastic_index.settings. These are
used upon index creation.
